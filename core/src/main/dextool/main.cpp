...
int main2() {
    ...
    const char *path = "/data/data/eggfly.kvm/classes.dex";
    ...
    const u1 *pEncodedData = dexUtil->dexGetClassData(*classDef0);
    DexClassData *pClassData = dexUtil->dexReadAndVerifyClassData(&pEncodedData, NULL);
    if (!pClassData) {
        ALOGI("no class data");
    }

    const char *methodName = dexUtil->getMethodName(classDef0->classIdx, 0);
    ALOGI("vMethods count: %d\n", pClassData->header.virtualMethodsSize);
    ALOGI("vMethod name: %s\n", methodName);
