...
int main(int argc, char *argv[]) {
    freopen("/data/data/eggfly.kvm/log.txt", "w", stdout);
    ...
    for (c = 1; c <= (int) *header.class_defs_size; c++) { /*run through all the class */
        ...
        offset = *header.class_defs_off +
                 ((c - 1) * sizeof(class_def_item)); /*get the offset for this class definition*/
        fseek(input, offset, SEEK_SET);
        printf("[] Class %d ", c);
        fread(&class_def_item, 1, sizeof(class_def_item),
              input); /*read the class definition from the input*/
        ...
        if (DEBUG) {
            ...
            printf("\taccess_flags='0x%x':",
                   *class_def_item.access_flags); /*need to interpret this*/
            parseAccessFlags(*class_def_item.access_flags);
            printf("\tsuperclass_idx='0x%x':", *class_def_item.superclass_idx);
            printTypeDesc(string_id_list, type_id_list, *class_def_item.superclass_idx, input, str,
                          "%s\n");
            ...
            if (*class_def_item.source_file_idx != NO_INDEX)
                printStringValue(string_id_list, *class_def_item.source_file_idx, input, str,
            ...
            printf("\tclass_data_off=0x%x (%d)\n", *class_def_item.class_data_off,
                   *class_def_item.class_data_off);
            printf("\tstatic_values_off=0x%x (%d)\n", *class_def_item.static_values_off,
                   *class_def_item.static_values_off);
        }
        ...
        if (!DEBUG)
            printf("%d direct methods, %d virtual methods\n", direct_methods_size,
